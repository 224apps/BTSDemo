...
-(void)initializeFileDownloadDataArray{
    ...
    [self.arrFileDownloadData addObject:[[FileDownloadObject alloc] initWithFileTitle:@"iOS Programming Guide" andSourceInfo:@"https://developer.apple.com/library/ios/documentation/iphone/conceptual/iphoneosprogrammingguide/iphoneappprogrammingguide.pdf"]];
    [self.arrFileDownloadData addObject:[[FileDownloadObject  alloc] initWithFileTitle:@"Human Interface Guidelines" andSourceInfo:@"https://developer.apple.com/library/ios/documentation/UserExperience/Conceptual/MobileHIG/MobileHIG.pdf"]];
    [self.arrFileDownloadData addObject:[[FileDownloadObject  alloc] initWithFileTitle:@"Networking Overview" andSourceInfo:@"https://developer.apple.com/library/ios/documentation/NetworkingInternetWeb/Conceptual/NetworkingOverview/NetworkingOverview.pdf"]];
    [self.arrFileDownloadData addObject:[[FileDownloadObject  alloc] initWithFileTitle:@"AV Foundation" andSourceInfo:@"https://developer.apple.com/library/ios/documentation/AudioVideo/Conceptual/AVFoundationPG/AVFoundationPG.pdf"]];
    [self.arrFileDownloadData addObject:[[FileDownloadObject  alloc] initWithFileTitle:@"iPhone User Guide" andSourceInfo:@"http://manuals.info.apple.com/MANUALS/1000/MA1565/en_US/iphone_user_guide.pdf"]];
}
...
-(int)getFileDownloadInfoIndexWithTaskIdentifier:(unsigned long)taskIdentifier{
    ...
    for (int i=0; i<[self.arrFileDownloadData count]; i++) {
        FileDownloadObject *fdi = [self.arrFileDownloadData objectAtIndex:i];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    FileDownloadObject *fdi = [self.arrFileDownloadData objectAtIndex:indexPath.row];
    ...
    // Set the file title.
    displayedTitle.text = fdi.titleOfTheFile;
...
- (IBAction)start:(id)sender {
    if ([[[[sender superview] superview] superview] isKindOfClass:[UITableViewCell class]]) {
        ...
        NSIndexPath *cellIndexPath = [self.tblView indexPathForCell:containerCell];
        int cellIndex = cellIndexPath.row;
        FileDownloadObject *fdi = [self.arrFileDownloadData objectAtIndex:cellIndex];
        ...
        if (!fdi.isDownloading) {
            ...
            if (fdi.taskIdentifier == -1) {
                ...
                fdi.downloadTask = [self.session downloadTaskWithURL:[NSURL URLWithString:fdi.sourceInfo]];
        ...
        [self.tblView reloadRowsAtIndexPaths:@[cellIndexPath] withRowAnimation:UITableViewRowAnimationNone];
    }
}
...
-(void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location{
    ...
    if (success) {
        ...
        FileDownloadObject *fdi = [self.arrFileDownloadData objectAtIndex:index];
        ...
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
            ...
            [self.tblView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:index inSection:0]]
...
-(void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didWriteData:(int64_t)bytesWritten totalBytesWritten:(int64_t)totalBytesWritten totalBytesExpectedToWrite:(int64_t)totalBytesExpectedToWrite{
    ...
    if (totalBytesExpectedToWrite == NSURLSessionTransferSizeUnknown) {
        ...
        [[NSOperationQueue mainQueue] addOperationWithBlock:^{
            ...
            UITableViewCell *cell = [self.tblView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
            UIProgressView *progressView = (UIProgressView *)[cell viewWithTag:CellProgressBarTagValue];
            progressView.progress = fdi.downloadProgress;
        }];
    }
